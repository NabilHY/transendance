...
export const TournamentBracket: React.FC<TournamentBracketProps> = ({ 
  ...
  useEffect(() => {
    const fetchAvatars = async () => {
      ...
      for (const userId of uniqueUserIds) {
        try {
          await ensureCsrf();
          const user = await umGetUser(userId);
          if (user) {
  ...
  const quarterFinals = bracket.filter(m => m.stage === 'quarter');
  const semiFinals = bracket.filter(m => m.stage === 'semi');
    ...
    <div style={styles.container}>
      ...
      <div style={styles.bracketContainer}>
        ...
        <div style={styles.stage}>
          ...
          {quarterFinals.map((match, idx) => renderMatch(match, idx))}
        </div>
        ...
        <div style={styles.stage}>
          ...
          {semiFinals.map((match, idx) => renderMatch(match, idx))}
        </div>
